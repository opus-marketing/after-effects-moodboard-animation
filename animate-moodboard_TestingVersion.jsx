...
var dialog = createDockableUI(this);
    dialog.text = "Animate Moodboards TESTING v0.9 08:50"; 
...
dialog.time.offset.offsetBeats.onChange = function() { beatsOffset = this.text; }
...
function createMaskedLayers () {
    ...
    var compWidth = curItem.width;//1920;
    var compHeight = curItem.height;//1080;
...
function animateMaskedLayer(layer, h, compH, compW, t) {
    ...
    // layer.position.setValueAtTime(0, 50);
}
...
function offsetLayer() {
    ...
    while( i < layers.length ) {
        ...
        layers[i].inPoint = inP;
        layers[i].outPoint = inP+;
        i++;
    }
}